...
export const reserve = internalMutation({ args: { workflow: workflowValidator, modelId: v.string(), poolIds: v.array(v.id('debugLiveGeminiPools')), requestCount: v.number() }, handler: async (ctx, args): Promise<{ apiKey?: string; reservationId?: Id<'debugLiveGeminiReservations'>; cacheName?: string; error?: string }> => {
  ...
  const now = Date.now(); const registry = keyRegistry(); const candidates = await Promise.all(args.poolIds.map(async (poolId) => ({ pool: await ctx.db.get(poolId), limit: await ctx.db.query('debugLiveGeminiQuotaLimits').withIndex('by_pool_and_model', (q) => q.eq('poolId', poolId).eq('modelId', args.modelId)).unique() })))
  for (const candidate of candidates) {
    if (candidate.pool === null || candidate.limit === null || !candidate.pool.enabled || registry[candidate.pool.keyAlias] === undefined) continue
    const dayKey = dateKey(now, candidate.pool.resetTimezone); const [recent, today] = await Promise.all([ctx.db.query('debugLiveGeminiReservations').withIndex('by_pool_model_and_requestedAt', (q) => q.eq('poolId', candidate.pool!._id).eq('modelId', args.modelId).gte('requestedAt', now - 60_000)).collect(), ctx.db.query('debugLiveGeminiReservations').withIndex('by_pool_model_day', (q) => q.eq('poolId', candidate.pool!._id).eq('modelId', args.modelId).eq('dayKey', dayKey)).collect()])
    ...
    return { apiKey: registry[candidate.pool.keyAlias], reservationId }
  }
